...
marker = "Rozpoznawane są również typowe warianty nazw, np. `ID obiektu`, `Klient`, `Nazwa budowy` i `Godzina`."
note = "Opcjonalne kolumny `CzasDojazdu` i `CzasPowrotu` podają czasy w minutach i są wczytywane bezpośrednio do roboczych czasów budowy. Jeśli tych kolumn nie ma, importer zachowuje dotychczasowe działanie."
if note not in readme_text:
    if marker not in readme_text:
        raise RuntimeError("README.md: nie znaleziono miejsca na opis czasów przejazdu")
    readme.write_text(readme_text.replace(marker, note + "\n\n" + marker, 1), encoding="utf-8")
