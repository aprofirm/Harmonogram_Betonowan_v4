...
`StartRoboczy` ani kursów gruszek. Cały Etap 4, wraz z testem operatora, jest zakończony. Punkt **5A** zabezpiecza centralny przebieg, a **5B.1** po raz pierwszy stosuje rzeczywiście możliwy start pompy wyłącznie do `StartRoboczy`, bez zmiany planu źródłowego i decyzji operatora. Wersja webowa jest publikowana z `main` pod adresem `https://aprofirm.github.io/Harmonogram_Betonowan_v4/`. Następny krok to **5B.2 — jawny konflikt braku możliwej pompy**.
