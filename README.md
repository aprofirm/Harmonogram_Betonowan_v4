...
`StartRoboczy` ani kursów gruszek. Cały Etap 4, wraz z testem operatora, jest zakończony. Punkt **5B.1** stosuje możliwy start pompy do `StartRoboczy`, a **5B.2** zwraca jawny konflikt dla braku zasobu, zbyt małego wysięgu, braku trasy lub niedostępności bez wymyślania zastępczej godziny. Wersja webowa jest publikowana z `main` pod adresem `https://aprofirm.github.io/Harmonogram_Betonowan_v4/`. Następny krok to **5B.3 — testy propagacji skutku pompy**.
